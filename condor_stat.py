...
def format_table(
    user_stats: defaultdict, only: str, user_priorities: dict[str, float], current_user: Optional[str] = None
) -> PrettyTable:
    """Format job statistics into a table."""
    headers = _get_headers(only)
    current_date = datetime.datetime.now().strftime("%d/%m")
    tab = PrettyTable(headers, align="l", hrules=1)

    # Create context object to reduce parameter passing
    ctx = TableContext(
        current_user=current_user or "", current_date=current_date, only=only, user_priorities=user_priorities
    )
...
if __name__ == "__main__":
    ...
    table = format_table(user_stats, args.only, user_priorities, current_user=username)
